...
class HRForgeApp {
  ...
  constructor(options = {}) {
    ...
    this.postings = [];
    this.activeInterviewPostingId = '';
    this.interviewStorageKey = 'hrforge.interviewSetup.v1';
    this.interviewStore = {};
  }

  init() {
    this._loadInterviewStore();
    ...
    document.querySelectorAll('.nav-item').forEach(item => {
      item.addEventListener('click', e => {
        ...
        const titles = {
          ...
          interview:['Interview Setup', 'Track applicants, ATS-qualified candidates, and interview letters by posted job ID'],
        };
        ...
        const showComposeActions = tab === 'compose';
        if (generateBtn) generateBtn.style.display = showComposeActions ? 'inline-flex' : 'none';
        if (aiBadge) aiBadge.style.display = showComposeActions ? 'inline-flex' : 'none';

        if (tab === 'interview') {
          this._loadInterviewPostings();
        }
      });
    });
  }
  ...
  _loadInterviewStore() {
    try {
      const raw = localStorage.getItem(this.interviewStorageKey);
      this.interviewStore = raw ? JSON.parse(raw) : {};
      if (!this.interviewStore || typeof this.interviewStore !== 'object') this.interviewStore = {};
    } catch {
      this.interviewStore = {};
    }
  }
  ...
  _saveInterviewStore() {
    localStorage.setItem(this.interviewStorageKey, JSON.stringify(this.interviewStore));
  }

  _jobCode(posting) {
    const token = (posting?.id || '').slice(0, 8).toUpperCase();
    return token ? `JOB-${token}` : 'JOB-UNASSIGNED';
  }

  _getActivePosting() {
    return this.postings.find(p => p.id === this.activeInterviewPostingId) || null;
  }

  _getPostingBucket(postingId, createIfMissing = false) {
    if (!postingId) return null;
    if (!this.interviewStore[postingId] && createIfMissing) {
      this.interviewStore[postingId] = { applicants: [] };
    }
    return this.interviewStore[postingId] || null;
  }

  _getSelectedCandidateIds() {
    const checked = [...document.querySelectorAll('.int-select:checked')];
    return [...new Set(checked.map(el => el.dataset.id).filter(Boolean))];
  }

  _generateLetterText(type, candidates, posting, custom = {}) {
    const jobCode = this._jobCode(posting);
    const role = posting?.role || 'the position';
    const company = posting?.company?.name || 'HR Forge';
    const lines = [];

    if (type === 'custom') {
      const heading = custom.heading || 'Candidate Update';
      const body = custom.body || 'Thank you for your application for {role}. Your Job ID is {jobId}.';
      lines.push(`${heading} - ${role} (${jobCode})`);
      lines.push('');
      candidates.forEach((c, idx) => {
        const msg = body
          .replaceAll('{name}', c.name)
          .replaceAll('{role}', role)
          .replaceAll('{jobId}', jobCode)
    ...
    lines.push(`Interview Confirmation - ${role} (${jobCode})`);
    lines.push(`Company: ${company}`);
    lines.push('');
    candidates.forEach((c, idx) => {
      lines.push(`${idx + 1}. ${c.name} | Score: ${c.score} | Email: ${c.email || 'N/A'} | Phone: ${c.phone || 'N/A'}`);
      lines.push(`   Dear ${c.name},`);
      lines.push(`   You have been shortlisted after ATS review for the ${role} role (${jobCode}).`);
      lines.push('   This message confirms your interview stage selection. Our HR team will share schedule details shortly.');
      lines.push('');
    });
    return lines.join('\n').trim();
  }
  ...
  _bindInterview() {
    const postingSel = document.getElementById('interviewPostingSelect');
    if (!postingSel) return;

    postingSel.addEventListener('change', () => {
      this.activeInterviewPostingId = postingSel.value;
      this._renderInterviewBoard();
    });

    document.getElementById('refreshInterviewPostingsBtn')?.addEventListener('click', () => {
      this._loadInterviewPostings(true);
    });

    document.getElementById('addApplicantBtn')?.addEventListener('click', () => {
      const posting = this._getActivePosting();
      if (!posting) {
        UIHelpers.toast('Select a posted job first', 'error');
        return;
      }
      ...
      const scoreRaw = document.getElementById('intScore').value;
      const score = Math.max(0, Math.min(100, parseInt(scoreRaw || '0', 10) || 0));
      const calledRequested = !!document.getElementById('intCalled').checked;

      if (!name) {
        UIHelpers.toast('Candidate name is required', 'error');
        return;
      }

      const atsPassed = score >= 80;
      const bucket = this._getPostingBucket(this.activeInterviewPostingId, true);
      bucket.applicants.unshift({
        ...
        score,
        atsPassed,
        calledForInterview: atsPassed ? calledRequested : false,
        createdAt: new Date().toISOString(),
      });

      this._saveInterviewStore();
      this._renderInterviewBoard();
      ['intName', 'intEmail', 'intPhone', 'intScore'].forEach(id => { document.getElementById(id).value = ''; });
      document.getElementById('intCalled').checked = true;
      UIHelpers.toast('Applicant added to selected job', 'success');
    });

    document.getElementById('markSelectedCalledBtn')?.addEventListener('click', () => {
      const ids = this._getSelectedCandidateIds();
      if (!ids.length) {
        UIHelpers.toast('Select candidate(s) first', 'error');
        return;
      }

      const bucket = this._getPostingBucket(this.activeInterviewPostingId);
      if (!bucket) return;

      let updated = 0;
      bucket.applicants = bucket.applicants.map(c => {
        if (ids.includes(c.id) && c.atsPassed) {
          updated += 1;
          return { ...c, calledForInterview: true };
        }
        return c;
      });
      ...
      this._saveInterviewStore();
      this._renderInterviewBoard();
      UIHelpers.toast(updated ? `Marked ${updated} candidate(s) as interview called` : 'Only ATS-passed candidates can be marked called', updated ? 'success' : 'error');
    });

    document.getElementById('draftConfirmationBtn')?.addEventListener('click', () => {
      const posting = this._getActivePosting();
      const ids = this._getSelectedCandidateIds();
      if (!posting || !ids.length) {
        UIHelpers.toast('Select candidate(s) first', 'error');
        return;
      }
      const bucket = this._getPostingBucket(this.activeInterviewPostingId);
      const picked = bucket.applicants.filter(c => ids.includes(c.id));
      document.getElementById('interviewLetterOut').value = this._generateLetterText('confirmation', picked, posting);
      UIHelpers.toast('Confirmation letter draft ready', 'success');
    });

    document.getElementById('draftCustomBtn')?.addEventListener('click', () => {
      const posting = this._getActivePosting();
      const ids = this._getSelectedCandidateIds();
      if (!posting || !ids.length) {
        UIHelpers.toast('Select candidate(s) first', 'error');
      ...
      const heading = window.prompt('Custom letter title', 'Custom Candidate Update');
      if (heading === null) return;
      const body = window.prompt('Custom body (use {name}, {role}, {jobId}, {company})', 'Dear {name}, this is an update regarding {role} ({jobId}) at {company}.');
      if (body === null) return;

      const bucket = this._getPostingBucket(this.activeInterviewPostingId);
      const picked = bucket.applicants.filter(c => ids.includes(c.id));
      document.getElementById('interviewLetterOut').value = this._generateLetterText('custom', picked, posting, { heading, body });
      UIHelpers.toast('Custom letter draft ready', 'success');
    });

    document.getElementById('copyLetterBtn')?.addEventListener('click', async () => {
      const out = document.getElementById('interviewLetterOut');
      const text = out.value.trim();
      if (!text) {
        UIHelpers.toast('No letter draft to copy', 'error');
        return;
      }
      try {
        await navigator.clipboard.writeText(text);
        UIHelpers.toast('Draft copied to clipboard', 'success');
      } catch {
        out.select();
        document.execCommand('copy');
        UIHelpers.toast('Draft copied', 'success');
      }
    });

    document.getElementById('selectAllApplicants')?.addEventListener('change', e => {
      document.querySelectorAll('#allApplicantsBody .int-select').forEach(cb => {
        cb.checked = e.target.checked;
      });
    });

    document.getElementById('selectAllShortlisted')?.addEventListener('change', e => {
      document.querySelectorAll('#shortlistedBody .int-select').forEach(cb => {
        cb.checked = e.target.checked;
      });
    });
  ...
  async _loadInterviewPostings(showToast = false) {
    const sel = document.getElementById('interviewPostingSelect');
    if (!sel) return;

    try {
      const res = await BackendAPI.listPostings();
      this.postings = Array.isArray(res?.data) ? res.data : [];

      if (!this.postings.length) {
        this.activeInterviewPostingId = '';
        sel.innerHTML = '<option value="">No saved job postings yet</option>';
        this._renderInterviewBoard();
        if (showToast) UIHelpers.toast('No saved jobs found. Save a posting from Preview first.', 'error');
        return;
      }

      sel.innerHTML = this.postings.map(p => {
        const code = this._jobCode(p);
        const role = p.role || 'Untitled Role';
        const dept = p.department || 'No Department';
        return `<option value="${p.id}">${code} - ${role} (${dept})</option>`;
      }).join('');

      const stillExists = this.postings.some(p => p.id === this.activeInterviewPostingId);
      this.activeInterviewPostingId = stillExists ? this.activeInterviewPostingId : this.postings[0].id;
      sel.value = this.activeInterviewPostingId;
      this._renderInterviewBoard();
      if (showToast) UIHelpers.toast('Posted jobs refreshed', 'success');
    } catch (err) {
      this.postings = [];
      this.activeInterviewPostingId = '';
      sel.innerHTML = '<option value="">Could not load posted jobs</option>';
      this._renderInterviewBoard();
      if (showToast) UIHelpers.toast(`Failed to load jobs: ${err.message}`, 'error');
    }
  }

  _renderInterviewBoard() {
    const posting = this._getActivePosting();
    const allBody = document.getElementById('allApplicantsBody');
    const shortBody = document.getElementById('shortlistedBody');

    if (!posting) {
      document.getElementById('intStatJobId').textContent = '—';
      document.getElementById('intStatRole').textContent = '—';
      document.getElementById('intStatAll').textContent = '0';
      document.getElementById('intStatShort').textContent = '0';
      if (allBody) allBody.innerHTML = '<tr><td colspan="5" class="table-empty">Select a posted job to manage applicants.</td></tr>';
      if (shortBody) shortBody.innerHTML = '<tr><td colspan="4" class="table-empty">No shortlisted candidates yet.</td></tr>';
      return;
    }

    const bucket = this._getPostingBucket(this.activeInterviewPostingId, true);
    const applicants = bucket.applicants || [];
    const shortlisted = applicants.filter(c => c.atsPassed && c.calledForInterview);

    document.getElementById('intStatJobId').textContent = this._jobCode(posting);
    document.getElementById('intStatRole').textContent = posting.role || '—';
    document.getElementById('intStatAll').textContent = String(applicants.length);
    document.getElementById('intStatShort').textContent = String(shortlisted.length);

    if (!applicants.length) {
      allBody.innerHTML = '<tr><td colspan="5" class="table-empty">No applicants added for this job yet.</td></tr>';
    } else {
      allBody.innerHTML = applicants.map(c => `
        <tr>
          <td><input type="checkbox" class="int-select" data-id="${c.id}" /></td>
          <td>
            <div><strong>${c.name}</strong></div>
            <div class="int-contact">${c.email || 'No email'} · ${c.phone || 'No phone'}</div>
          </td>
          <td>${c.score}</td>
          <td>
            <label style="display:flex;align-items:center;gap:.35rem;">
              <input type="checkbox" class="int-ats-toggle" data-id="${c.id}" ${c.atsPassed ? 'checked' : ''} />
              <span class="int-chip ${c.atsPassed ? 'pass' : 'fail'}">${c.atsPassed ? 'Pass' : 'Fail'}</span>
            </label>
          </td>
          <td><input type="checkbox" class="int-called-toggle" data-id="${c.id}" ${c.calledForInterview ? 'checked' : ''} ${c.atsPassed ? '' : 'disabled'} /></td>
        </tr>
      `).join('');
    }

    if (!shortlisted.length) {
      shortBody.innerHTML = '<tr><td colspan="4" class="table-empty">No ATS-passed and interview-called candidates yet.</td></tr>';
    } else {
      shortBody.innerHTML = shortlisted.map(c => `
        <tr>
          <td><input type="checkbox" class="int-select" data-id="${c.id}" /></td>
          <td><strong>${c.name}</strong></td>
          <td>${c.score}</td>
          <td class="int-contact">${c.email || 'No email'}<br/>${c.phone || 'No phone'}</td>
        </tr>
      `).join('');
    }

    document.querySelectorAll('.int-ats-toggle').forEach(el => {
      el.addEventListener('change', () => {
        const id = el.dataset.id;
        bucket.applicants = bucket.applicants.map(c => {
          if (c.id !== id) return c;
          const nextPassed = el.checked;
          return { ...c, atsPassed: nextPassed, calledForInterview: nextPassed ? c.calledForInterview : false };
        });
        this._saveInterviewStore();
        this._renderInterviewBoard();
      });
    ...
    document.querySelectorAll('.int-called-toggle').forEach(el => {
      el.addEventListener('change', () => {
        const id = el.dataset.id;
        bucket.applicants = bucket.applicants.map(c => c.id === id ? { ...c, calledForInterview: !!el.checked } : c);
        this._saveInterviewStore();
        this._renderInterviewBoard();
      });
    });
...
const BackendAPI = {
  ...
  /** Post to Betopia Jobs portal via server bridge */
  async postToBetopia(payload) {
    return this._fetch("/api/post/betopia", {
      method: "POST",
      body: JSON.stringify(payload),
    });
  },

  /** Post to configurable external job portal via server bridge */
  async postToJobPortal(payload) {
    return this._fetch("/api/post/job-portal", {
      method: "POST",
      body: JSON.stringify(payload),
    });
  },
...
function _injectSocialButtons(integrations) {
  ...
  // Betopia Jobs button (manual portal submit helper)
  const betopiaBtn = document.createElement("button");
  betopiaBtn.className = "btn-tool";
  betopiaBtn.style.cssText = "background:#ffefe2;color:#d65b00;border-color:#ffb787";
  betopiaBtn.innerHTML = `Betopia Jobs`;
  betopiaBtn.addEventListener("click", async () => {
    const app = window.HRForge;
    const text = app.generatedOutputs.jd || document.getElementById("previewOutput").value;
    if (!text) { UIHelpers.toast("Generate a posting first", "error"); return; }
    UIHelpers.showSpinner("Opening Betopia Jobs portal…");
    try {
      const result = await BackendAPI.postToBetopia({
        text,
        posting_id: savedPostingId.current,
      });
      UIHelpers.hideSpinner();
      UIHelpers.toast("Betopia portal opened for posting", "success");
      if (result.url) window.open(result.url, "_blank");
    } catch (err) {
      UIHelpers.hideSpinner();
      UIHelpers.toast(`Betopia post error: ${err.message}`, "error");
    }
  });
  toolbar.appendChild(betopiaBtn);

  // External Job Portal button (URL from env or user prompt)
  const portalBtn = document.createElement("button");
  portalBtn.className = "btn-tool";
  portalBtn.innerHTML = `Post to Job Portal`;
  portalBtn.addEventListener("click", async () => {
    const app = window.HRForge;
    const text = app.generatedOutputs.jd || document.getElementById("previewOutput").value;
    if (!text) { UIHelpers.toast("Generate a posting first", "error"); return; }

    const configured = (integrations && integrations.job_portal_url) ? integrations.job_portal_url : "";
    const portalUrl = configured || window.prompt("Enter job portal URL", "https://");
    if (!portalUrl) return;

    UIHelpers.showSpinner("Opening job portal…");
    try {
      const result = await BackendAPI.postToJobPortal({
        text,
        posting_id: savedPostingId.current,
        url: portalUrl,
      });
      UIHelpers.hideSpinner();
      UIHelpers.toast("Job portal opened for posting", "success");
      if (result.url) window.open(result.url, "_blank");
    } catch (err) {
      UIHelpers.hideSpinner();
      UIHelpers.toast(`Job portal post error: ${err.message}`, "error");
    }
  });
  toolbar.appendChild(portalBtn);
}
